...
public final class LocalManyToOneFunctionInvokeAdapter<I, O> implements FunctionInvokeAdapter<I, O> {
                ...
                .onItem().transform(collected -> {
                    if (collected.size() > maxItems) {
                        throw new IllegalStateException(
                            "Function invoke overflow: received " + collected.size()
    ...
    private String computeIdempotencyKey(
        ...
        if (context.idempotencyPolicy() == IdempotencyPolicy.EXPLICIT) {
            return context.explicitIdempotencyKey()
                .orElseGet(() -> fallbackIdempotencyKey(traceId));
        }
        String fallback = fallbackIdempotencyKey(traceId);
        if (envelopes.isEmpty()) {
            return fallback;
        }
    ...
    private String fallbackIdempotencyKey(String traceId) {
        return traceId + ":" + outputPayloadModel + ":" + UUID.randomUUID();
    }
}
