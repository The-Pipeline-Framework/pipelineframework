...
| DB/API reads that affect business decisions | Captured query step: `LoadX -> CapturedX`, then pure decision step | DSL step, connector SPI, and guardrails | Very high | Makes mutable external state explicit, replayable, auditable, and cacheable. |
...
- The extension point is a connector, not a plugin: applications implement `QueryConnector<I, O>`.
...
- Query metadata includes query id, connector name, query version, input/output types, connector config, and capture key fields.
...
steps:
  ...
  - name: "Assess Customer Risk"
    ...
    cardinality: "ONE_TO_ONE"
    input: "com.example.CustomerRiskSnapshot"
...
- Current first-class MVP: medium-large, covering DSL parsing, generated query client step, connector SPI, in-memory capture store, and captured replay semantics.
...
- A template generator can expose query reads as structured choices: connector, query id, version, input/output DTOs, connector config, and capture key fields.
