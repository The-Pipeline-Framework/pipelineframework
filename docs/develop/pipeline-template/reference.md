# Complete Pipeline Template DSL Reference

This is the exhaustive syntax and compatibility reference. For a task-oriented introduction that
connects the template to TPF's Functional Core, Imperative Shell model, start with the
...
```mermaid
flowchart LR
    T[Canonical types] --> P[Typed steps]
    P --> C[Compiler validation]
    C --> G[Generated contracts and adapters]
```
...
For the complete packaged-composition journey, see the [Blocks Guide](/develop/blocks/).
...
The [GraphQL Connector and Blocks](/develop/extension/graphql-connector) proof applies this split to a
...
See the [Pipeline Compilation Guide](/develop/pipeline-compilation/) for build-time validation and generated artifacts.
