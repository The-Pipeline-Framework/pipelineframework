...
1. [Boundary Taxonomy](/versions/v26.6.2/evolve/brokered-boundaries/boundary-taxonomy) maps broker concepts into TPF runtime boundaries.
2. [Dispatch Substrates](/versions/v26.6.2/evolve/brokered-boundaries/dispatch-substrates) explains why Kafka is a substrate, not a TPF mode.
3. [Envelope And Data Policy](/versions/v26.6.2/evolve/brokered-boundaries/envelope-and-data-policy) separates loose payloads from strict TPF control metadata.
4. [Adoption And Slices](/versions/v26.6.2/evolve/brokered-boundaries/adoption-and-slices) captures the value proposition and implementation ordering.

## Naming

Use **step host** for an external process, service, pod, or function that executes a TPF step boundary.

Keep **transition worker** only for the existing durable coordinator concept described in [Worker Protocols](/versions/v26.6.2/evolve/durable-coordinator/worker-protocols). Avoid using "worker" as a generic synonym in this guide because it clashes with existing runtime worker terminology.
