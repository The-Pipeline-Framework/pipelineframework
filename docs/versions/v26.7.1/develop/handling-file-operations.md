...
Use a connector when the application needs to admit files or object-store entries as pipeline input. Start with [Object Ingest](/versions/v26.7.1/design/object-ingest) and use this page only for request/response file endpoints that are truly custom application APIs.
