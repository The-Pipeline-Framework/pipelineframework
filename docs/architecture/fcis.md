...
The [Pipeline Template Guide](/develop/pipeline-template/) is the authoring front door to this split:
`pipeline.yaml` names the domain contract and flow, typed Java functions make the decisions, and TPF
generates the shell around both.
...
| Admit files, object-store entries, or external payloads | connectors such as [Object Ingest](/architecture/object-ingest) |
| Wait for human approval, webhook callbacks, provider responses, or long-running jobs | [Await Boundaries](/architecture/await-boundaries) |
...
| Call a synchronous existing method or remote request/response endpoint | [Operators](/architecture/operators) |
| Store business outputs for later query or audit | [Persistence](/architecture/persistence) |
| Reuse expensive derived outputs | [Caching](/architecture/caching/) |
| Hand off stable output to a separately owned downstream pipeline | [Checkpoint Handoff](/deploy/orchestrator-runtime/checkpoint-handoff) |
