...
For modeling guidance, start with [Await Boundaries](/versions/v26.6.2/design/await-boundaries). For production operation, see [Await Boundary Operations](/versions/v26.6.2/operate/await-boundaries). Internally, await is backed by durable await units; for implementation diagrams and the state model, see [Await Unit Runtime](/versions/v26.6.2/evolve/await-unit-runtime/).
