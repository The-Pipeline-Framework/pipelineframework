...
- [OpenAPI Capability Proof](https://github.com/The-Pipeline-Framework/pipelineframework/blob/main/examples/openapi-capability-proof/README.md) — imports two `POST` operations as an ordinary Query and Command, runs them directly and through the packaged callable loop, and proves the runtime contains pins rather than an OpenAPI parser or source document.
