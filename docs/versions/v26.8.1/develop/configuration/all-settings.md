...
See [Field Materialization](/versions/v26.8.1/design/materialization) for the YAML surface.
...
| `pipeline.max-recursive-depth` | integer | `64` | Maximum number of direct self-recursive local pipeline invocations below a root invocation. The first call beyond this depth fails without retry. |
...
Kafka await also requires SmallRye channel configuration for the framework-owned `tpf-await-kafka-requests` outgoing channel and `tpf-await-kafka-responses` incoming channel. See [Await Boundaries](/versions/v26.8.1/deploy/orchestrator-runtime/await).
