...
```mermaid
flowchart LR
    O[File or object-store entry] --> I[Object Ingest Connector]
    I --> P[Typed Pipeline]
    P --> U[Object Publish Connector]
    U --> D[Durable object]
```

For the common connector authoring model and current catalogue, see the [Connectors Guide](/develop/connectors/).
...
- CSV Payments connector-owned input/output path:
  [`config/pipeline.yaml`](https://github.com/The-Pipeline-Framework/csv-kafka-payments/blob/main/config/pipeline.yaml)
