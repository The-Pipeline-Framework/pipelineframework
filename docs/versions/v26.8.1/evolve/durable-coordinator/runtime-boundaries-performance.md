...
For the role split behind the terms coordinator, transition worker, and `orchestrator-svc`, start with [Coordinator And Worker Topology](/versions/v26.8.1/evolve/durable-coordinator/coordinator-worker-topology).
...
The third diagram is future work, not current `FUNCTION` support. The design spike is [All-Serverless Durable Coordinator](/versions/v26.8.1/evolve/durable-coordinator/all-serverless-coordinator).
