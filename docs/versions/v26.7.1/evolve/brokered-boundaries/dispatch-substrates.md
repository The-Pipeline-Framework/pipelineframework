...
- [Step-Aware Invocation Runtime](/versions/v26.7.1/evolve/durable-coordinator/boundary-invocation-model) defines shared invocation points for step, transition-worker, and transport-boundary execution.
- [Worker Protocols](/versions/v26.7.1/evolve/durable-coordinator/worker-protocols) already model transition-worker command/result envelopes over local, REST, gRPC, and SQS.
- [Await Unit Runtime](/versions/v26.7.1/evolve/await-unit-runtime/) defines durable suspend/resume semantics that can use different transports.
- [Immutable Segment And Boundary Model](/versions/v26.7.1/evolve/await-unit-runtime/immutable-boundaries) defines the internal fact model shared by await completion and checkpoint handoff admissions.
- [Checkpoint Handoff](/versions/v26.7.1/deploy/orchestrator-runtime/checkpoint-handoff) is the natural user-facing shape for pipeline-to-pipeline publication.
- [Runtime Core Decoupling](/versions/v26.7.1/evolve/runtime-core-decoupling) keeps runtime-adapter concerns out of core semantics.
