...
/**
 * Debug test to trace what's happening in the actual pipeline step processing
 */
class DebugPipelineStepProcessingTest {

    private static final String PROTOBUF_DESCRIPTOR_FILE = "protobuf.descriptor.file";

    @Test
    void debugPipelineStepProcessing() throws Exception {
        ...
        JavaFileObject mutinyProcessFolderServiceGrpcSource = JavaFileObjects.forSourceString(
            ...
            public class MutinyProcessFolderServiceGrpc {
                ...
                public static abstract class ProcessFolderServiceImplBase {
                    public io.grpc.stub.StreamObserver<InputCsvFileProcessingSvc.CsvFolder> remoteProcess(io.grpc.stub.StreamObserver<InputCsvFileProcessingSvc.CsvPaymentsInputFile> responseObserver) {
                        return null; // Mock implementation
                    }
                }
            }
        ...
        compilation.diagnostics().forEach(diagnostic -> System.out.println("Diagnostic: " + diagnostic));
        ...
        for (JavaFileObject fileObject : compilation.generatedSourceFiles()) {
            if (fileObject.getName().contains("ProcessFolderClientStep")) {
                ...
                // Check what types are being used
                if (sourceCode.contains("StepOneToMany<CsvFolder, CsvPaymentsInputFile>")) {
                    System.out.println("ERROR: Using domain types instead of gRPC types!");
                } else if (sourceCode.contains("StepOneToMany<")) {
                    System.out.println("INFO: Using some form of types in StepOneToMany");
                }
        ...
        if (!clientStepGenerated) {
            System.out.println("Client step was not generated!");
        }
        
        // Note: This test is just for debugging, so we don't assert success
