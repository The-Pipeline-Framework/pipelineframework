...
if [[ -f "$PROVENANCE_FILE" && -f "$FRAMEWORK_RUNTIME_JAR" \
  ...
  && "$(property_value 'framework.runtime.sha256')" \
    == "$("$ROOT_DIR/examples/csv-payments/hash-jar-content.sh" "$FRAMEWORK_RUNTIME_JAR")" ]]; then
...
FRAMEWORK_RUNTIME_SHA256="$("$ROOT_DIR/examples/csv-payments/hash-jar-content.sh" "$FRAMEWORK_RUNTIME_JAR")"
