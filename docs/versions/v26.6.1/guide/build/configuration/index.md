...
See [Field Materialization](/versions/v26.6.1/guide/plugins/materialization) for the YAML surface.
...
Kafka await also requires SmallRye channel configuration for the framework-owned `tpf-await-kafka-requests` outgoing channel and `tpf-await-kafka-responses` incoming channel. See [Await Boundaries](/versions/v26.6.1/guide/development/orchestrator-runtime/await).
