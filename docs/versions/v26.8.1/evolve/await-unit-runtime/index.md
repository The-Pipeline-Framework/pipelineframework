...
This guide is implementation-facing. Application-facing design guidance lives in [Await Boundaries](/versions/v26.8.1/design/await-boundaries). Runtime setup lives in [Await runtime setup](/versions/v26.8.1/deploy/orchestrator-runtime/await).

For the longer-term orchestration boundary that can move await units out of each app-hosted orchestrator, see [Durable Coordinator](/versions/v26.8.1/evolve/durable-coordinator/). For the immutable queue-async model that treats await completion and checkpoint handoff as the same boundary-admission shape, see [Immutable Segment And Boundary Model](/versions/v26.8.1/evolve/await-unit-runtime/immutable-boundaries).
...
1. [Model](/versions/v26.8.1/evolve/await-unit-runtime/) explains the durable records and cardinality semantics.
2. [Immutable Boundaries](/versions/v26.8.1/evolve/await-unit-runtime/immutable-boundaries) explains the segment, boundary, fact, and projection model behind queue-async.
3. [Sequences](/versions/v26.8.1/evolve/await-unit-runtime/sequences) shows unary, stream, aggregate, timeout, and resume flows.
4. [Patterns](/versions/v26.8.1/evolve/await-unit-runtime/patterns) explains the architectural patterns and why the unit model fixed the design.
5. [Limitations And Debt](/versions/v26.8.1/evolve/await-unit-runtime/operations-and-debt) tracks implementation limitations and follow-up work.
