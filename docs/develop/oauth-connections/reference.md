...
This is the detailed reference behind the [Experimental OAuth Connections Guide](./).
...
```mermaid
flowchart TB
    Q[TPF Query or Command] --> C[ConnectorExecutionContext]
    C --> R[Application ConnectionResolver]
    R --> H[Host security and connection infrastructure]
    H --> S[Authenticated SDK client]
...
[Import MCP tools as Connector operations](../extension/mcp-connector-import.md).
