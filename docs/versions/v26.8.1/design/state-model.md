...
1. Query business data later? Use [Persistence](/versions/v26.8.1/design/persistence).
2. Want to avoid recomputing a deterministic output? Use [Caching](/versions/v26.8.1/design/caching/).
3. Carrying large content across boundaries? Use [Field Materialization](/versions/v26.8.1/design/materialization).
4. Should work survive crashes? Use [Queue-Async Runtime](/versions/v26.8.1/deploy/orchestrator-runtime/queue-async).
5. Waiting for external reality? Use [Await Boundaries](/versions/v26.8.1/design/await-boundaries).
6. Handing off stable ownership to another pipeline? Use [Checkpoint Handoff](/versions/v26.8.1/deploy/orchestrator-runtime/checkpoint-handoff).
