---
title: Fan-out and Reduction
---

# Fan-out and Reduction
...
```mermaid
flowchart LR
    A[ONE_TO_ONE] --> B[ONE_TO_MANY]
    B --> C[MANY_TO_MANY]
    C --> D[MANY_TO_ONE / Reduction]
```

For authoring syntax and service shapes, use the
[Pipeline Template Guide](/develop/pipeline-template/).

The four-step shapes describe that behaviour in plain terms:
...
- `ONE_TO_MANY` steps increase the number of items in flight.
