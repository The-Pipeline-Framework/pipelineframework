...
For boundary-cost and runtime-mode tradeoffs, see [Runtime Boundaries And Performance](/versions/v26.7.1/evolve/durable-coordinator/runtime-boundaries-performance).
...
Use [Command Steps](/versions/v26.7.1/deploy/orchestrator-runtime/command) when the external side effect itself should be a managed pipeline boundary with a deterministic command id, effect log, duplicate policy, and recorded output replay.
