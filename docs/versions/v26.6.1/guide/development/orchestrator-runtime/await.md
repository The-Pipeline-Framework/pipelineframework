...
For production operation, see [Await Boundary Operations](/versions/v26.6.1/guide/operations/await-boundaries). Internally, await is backed by durable await units; for implementation diagrams and the state model, see [Await Unit Runtime](/versions/v26.6.1/guide/evolve/await-unit-runtime/).
...
For a deeper checkpoint handoff lifecycle guide, see [Checkpoint Handoff](/versions/v26.6.1/guide/development/orchestrator-runtime/checkpoint-handoff).
