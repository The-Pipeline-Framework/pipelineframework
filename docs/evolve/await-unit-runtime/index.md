...
Await units are the durable suspend/resume model behind the `await:` modifier on an ordinary authored operation. The unit records what operation result was dispatched, what completion is required, and what final payload should be replayed when the owning execution resumes.
...
3. [Sequences](/evolve/await-unit-runtime/sequences) shows unary, itemised-stream, timeout, and resume flows.
...
1. `AwaitUnitRecord`: one durable completion unit for a decorated operation at a specific execution and step index.
...
classDiagram
    ...
    AwaitCompletionSupport --> AwaitCoordinator : create / dispatch unit
...
`AwaitUnitRecord` is the current projection for completion of the authored operation. `AwaitInteractionRecord` is the transport-facing projection. The target queue-async model represents the same behaviour as immutable `BoundaryUnit` and `BoundaryInteraction` projections derived from appended facts.

In that model, `PipelineRunner` still runs synchronous step segments. A decorated operation suspends one `ExecutionSegment` by appending a `SegmentSuspended` fact. Every accepted completion appends `BoundaryCompletionAdmitted`. Only eligible itemised Kafka and SQS adapters, running in a worker shape that permits `LIVE_IF_SUPPORTED`, may hand that admitted completion to an active downstream `Multi`. Webhook and interaction-api completions append durable facts only. Without an eligible live session, the same durable facts create continuation segment work.
...
The operation retains its authored cardinality, but deferred completion applies once
to each result it emits.

| Emitted result shape | Unit shape | Interactions | Resume input |
| --- | --- | --- | --- |
| one result | one result, one completion | one primary interaction | scalar final output |
| result stream | one unit owning ordered item interactions | one interaction per emitted result | ordered final outputs |
| explicit collection result | one typed collection, one completion | one primary interaction | typed collection final output |

Whole-batch semantics are expressed by an explicit bounded canonical collection type,
not by an aggregate Await cardinality. Ordinary expansion or reduction steps own any
change between item and collection shape.
