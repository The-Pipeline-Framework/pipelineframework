...
This page is the canonical The Pipeline Framework (TPF) guide for `FUNCTION` platform builds that target Azure Functions. For the broader provider matrix, pair it with the [Multi-Cloud Function Providers Guide](/versions/v26.7.1/deploy/function-providers).
...
For the future all-serverless durable coordinator design track, see [All-Serverless Durable Coordinator](/versions/v26.7.1/evolve/durable-coordinator/all-serverless-coordinator). That design is not current Azure Functions support.
