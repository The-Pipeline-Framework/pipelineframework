---
title: Packaged segments are static composition imports
status: accepted
---

# ADR-0024: Packaged segments are static composition imports
...
validation, and release-identity authorities. Introducing a segment executor, runtime registry, or
...
A segment artifact publishes compiler-visible identity, artifact provenance, and one or more version
...
name is unique and does not collide with a local definition. Within one segment package, an internal short
...
A type contributed by a segment may bind its canonical identity to a package-owned Java class. That
...
- Installing a segment is a build dependency operation; there is no runtime download, registry, or
  discovery path.
- Segment implementation classes and transitive libraries use the application's normal classpath and
  dependency injection mechanisms.
- Imported and local definitions share one composition graph, one generated contract, and one runtime
  invocation mechanism. No parallel segment graph is generated.
...
- Packaging external authority, generalized segment parameters, and dynamic runtime loading remain
  out of scope.
